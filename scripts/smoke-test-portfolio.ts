...
async function getCurrentWheatPrice(): Promise<{ price: number; indexId: string } | null> {
  try {
    ...
    if (!wheatIndex) {
      console.error("❌ WHEAT index not found");
      return null;
    }
    ...
    if (!latestPrice) {
      console.error("❌ No price found for WHEAT");
      return null;
    }
    ...
    console.error("❌ Error getting WHEAT price:", error);
    return null;
  }
}
...
async function updateWheatPrice(token: string, newPrice: number): Promise<boolean> {
  try {
    const response = await fetch(`${API_BASE}/api/indexes/wheat/price`, {
      method: "POST",
      headers: {
    ...
    console.log(`✅ Updated WHEAT price to $${newPrice}/ton`);
    return true;
  } catch (error) {
...
async function createTestOption(
  ...
  try {
    ...
    const optionData = {
      title: `WHEAT CALL Option - Strike ${strike} (Test)`,
      type: "CALL" as const,
...
async function main() {
  ...
  // Step 2: Get current WHEAT price
  console.log("Step 2: Getting current WHEAT price...");
  const priceInfo = await getCurrentWheatPrice();
  if (!priceInfo) {
    console.error("❌ Failed to get price. Exiting.");
    process.exit(1);
  }
  console.log(`✅ Current WHEAT price: $${priceInfo.price}/ton\n`);
