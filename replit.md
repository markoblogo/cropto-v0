...
### Backend
The backend is an Express.js application in TypeScript, providing a RESTful JSON API. It features a flexible authentication system with Supabase (production) or file-based (`db.json`) for development, using bcrypt and JWT for security. **RBAC (Role-Based Access Control)** is implemented with three roles: `USER` (default for new registrations), `ADMIN` (elevated privileges), and `SUPER_ADMIN` (full system access). Email `a.biletskiy@gmail.com` is designated as SUPER_ADMIN. Three middleware functions enforce access control: `requireUser` (any authenticated user), `requireAdmin` (ADMIN or SUPER_ADMIN), and `requireSuperAdmin` (SUPER_ADMIN only). A database abstraction layer ensures transaction safety. Shared Zod schemas are used for data validation. A margin check job system calculates intrinsic value, P&L, triggers margin calls, and generates notifications. Security middleware blocks unauthorized Supabase service role key usage.
...
- **Authentication & Authorization**: JWT-based system with RBAC (Role-Based Access Control) and protected API endpoints. Three user roles: `USER` (default), `ADMIN`, `SUPER_ADMIN`. Login flow for ADMIN/SUPER_ADMIN users includes mode selection via `AdminModeSelectionModal` component, allowing choice between "Enter as User" (normal interface) or "Enter as Admin" (admin features). Selected mode stored in `localStorage` as `cropto_admin_mode`. Backend middleware (`requireUser`, `requireAdmin`, `requireSuperAdmin`) enforces granular access control. Migration script (`server/migrations/updateRolesSchema.ts`) available for converting legacy roles to new RBAC system.
  - **CRITICAL SECURITY REQUIREMENT**: When implementing admin features in the future, ALL admin UI/routes MUST check BOTH `localStorage.cropto_admin_mode === 'ADMIN'` AND `user.role ∈ {ADMIN, SUPER_ADMIN}` from authenticated session. Never trust localStorage alone - always verify backend role. All admin API endpoints MUST use `requireAdmin` or `requireSuperAdmin` middleware.
