...
export function MonitorHeader({ navItems }: MonitorHeaderProps) {
  return (
    <header className="sticky top-0 z-50 border-b border-border/60 bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/80">
      <div className="container mx-auto flex h-16 items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        <a href="#overview" className="flex items-center gap-3 rounded-md px-1 py-1 hover-elevate">
          ...
          <span className="hidden text-base font-semibold sm:inline">Cropto Monitor</span>
        </a>

        <nav className="hidden items-center gap-1 lg:flex" aria-label="Monitor sections">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
      ...
      <div className="border-t border-border/60 lg:hidden">
        <nav className="container mx-auto flex gap-2 overflow-x-auto px-4 py-2 sm:px-6" aria-label="Monitor sections mobile">
          {navItems.map((item) => (
            <a
